...
def analyze_algebraic_relations():
    ...
    # We have 16 variables: x0..x7 (input bits) and y0..y7 (output bits)
    # Total monomials of degree <= 2:
    # Degree 0: 1 (const)
    # Degree 1: 16 (x0..x7, y0..y7)
    # Degree 2: 120 (combinations of 2 different variables out of 16)
    # Total: 137
    num_monomials = 1 + 16 + 120

    # Construct the matrix M of size 256 x 137
    M_data = []
    for x_val in range(256):
        y_val = ABYSSAL_SBOX[x_val]
        
        # Get bits
        x_bits = [(x_val >> i) & 1 for i in range(8)]
        y_bits = [(y_val >> i) & 1 for i in range(8)]
        vars_list = x_bits + y_bits  # 16 elements
        
        row = []
        # Degree 0
        row.append(1)
        # Degree 1
        row.extend(vars_list)
        # Degree 2
        for i in range(16):
            for j in range(i + 1, 16):
                row.append(vars_list[i] * vars_list[j])
        
        M_data.append(row)

    # Convert to Galois field matrix
    M = GF(M_data)

    # Compute rank
    rank = np.linalg.matrix_rank(M)
    
    print("=== Algebraic Relation Search ===")
    print(f"Number of monomials of degree <= 2: {num_monomials}")
    print(f"Matrix Rank: {rank}")
    
    num_relations = num_monomials - rank
    print(f"Number of algebraic relations of degree <= 2: {num_relations}")
    if num_relations == 0:
        print("SUCCESS: The ABYSSAL S-box has NO algebraic relations of degree <= 2.")
    else:
        print(f"WARNING: The S-box has {num_relations} algebraic relations of degree <= 2.")
